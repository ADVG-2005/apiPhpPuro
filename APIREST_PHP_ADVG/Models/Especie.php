...
class Especie {
    ...
    public function create($data) {
        ...
        $stmt = $this->conn->prepare($query);
        return $stmt->execute([$data['nombre'], $data['descripcion'], $data['img'], $data['tiempoCrecimiento'], $data['fk_TiposEspecie']]);
    }

    public function update($id, $data) {
        $query = "UPDATE " . $this->table . " SET nombre = ?, descripcion = ?, img = ?, tiempoCrecimiento = ?, fk_TiposEspecie = ? WHERE id = ?";
        $stmt = $this->conn->prepare($query);
        return $stmt->execute([$data['nombre'], $data['descripcion'], $data['img'], $data['tiempoCrecimiento'], $data['fk_TiposEspecie'], $id]);
    }
